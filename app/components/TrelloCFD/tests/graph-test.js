...
test('numberOfCardsAtDate', (assert) => {
  ...
  assert.equals(
    numberOfCardsAtDate('4eea4ffc91e31d179270127c', '2016-02-12', data),
    3,
    'should return the correct number of card at given date'
  );
  assert.equals(
    numberOfCardsAtDate('563b1afeb758fc0e81a3c1b6', '2016-02-12', data),
    0,
    'should return 0 if there were no card of given list at given date'
  );
  assert.equals(
    numberOfCardsAtDate('4eea4ffc91e31d179270127c', '2013-08-01', data),
    0,
    'should return 0 if there were no card at given date'
  );
  assert.end();
});
...
test('parseToGraph', (assert) => {
  ...
  assert.looseEquals(
    parseToGraph(displayedLists, data),
    expected,
    'should correctly parse Trello data to be rendered'
  );
  assert.looseEquals(
    parseToGraph(displayedLists)(data),
    expected,
  ...
  assert.looseEquals(
    parseToGraph(displayedLists, null),
    [],
    'should return an empty list if no data is passed'
  );
  assert.looseEquals(
    parseToGraph(displayedLists, []),
    [],
    'should return an empty list if empty data is passed'
  );
  assert.looseEquals(
    parseToGraph(displayedLists, {}),
    [],
    'should return an empty list if data passed is an object'
  );
  assert.end();
});
