...
test('allLists', (assert) => {
  ...
  assert.looseEquals(
    allLists(data),
    ['Card Preparation [2]', 'Backlog', 'Icebox Énergie'],
    'should return the list of all unique lists from Trello data'
  );
  assert.looseEquals(
    allLists([]),
    [],
    'should return an empty list if data is empty'
  );
  assert.looseEquals(
    allLists([{}, {}]),
    [],
    'should return an empty list if data is missing content'
  );
  assert.end();
});
...
test('countCardsPerList', (assert) => {
  ...
  assert.looseEquals(
    countCardsPerList(R.identity, data),
    expectedWithIdentity,
    'should correctly count the number of cards per list'
  );
  assert.looseEquals(
    countCardsPerList(R.negate, data),
    expectedWithNegateParser,
    'should accept a numberOfCards parser as an argument'
  );
  assert.looseEquals(
    countCardsPerList(R.negate)(data),
    expectedWithNegateParser,
