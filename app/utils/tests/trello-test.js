  ...
  const expected = [
    {
      ...
      data: {
        ...
        card: {
          name: 'Erreur url Backbone lors de la remise en stock d’un props',
...
test('parseListName', (assert) => {
  assert.equals(
    parseListName('Card Preparation [4]'),
    'Card Preparation',
    'should trim trailing WIP indicator'
  );
  assert.equals(
    parseListName('Backlog'),
    'Backlog',
    'should leave a regular list name untouched'
  );
  assert.equals(
    parseListName('Live (March 2016)'),
    'Live (March 2016)',
    'should leave live list name untouched'
  );
  assert.end();
});
